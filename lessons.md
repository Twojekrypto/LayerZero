...
- **W `Flows` użytkownik najczęściej skanuje tylko trzy rzeczy: kto, ile i w którą stronę.** Kwoty USD, `Conviction/Pressure` jako tekst i długie opisy typu `External outflow` lepiej trzymać poza głównym wierszem. Jeśli te dane nie pomagają podjąć decyzji w 1-2 sekundy, powinny wypaść z tabeli i zostać tylko w drawerze albo tooltipie.
- **`Whale Transfers` czyta się najlepiej, gdy generyczne etykiety nie konkurują z prawdziwym sygnałem.** `Organic`, `Track/Watch` i placeholdery typu `Whale` jako główny label robią więcej szumu niż pożytku; lepiej pokazać konkretny byt lub adres, jeden mocny badge jakości (`Prime` / `High`) i ewentualnie balance odbiorcy.
- **Toolbar `Flows` powinien siedzieć tuż nad tabelami, a nie wysoko nad sekcją.** Search, cohort, chain, scope toggle i period pills działają najlepiej jako jedna kompaktowa listwa bezpośrednio nad `Top Accumulators / Top Sellers`; wtedy użytkownik widzi od razu, że steruje właśnie tym porównaniem, a nie całym blokiem statystyk ponad nim.
- **Summary w `Flows` powinno być insight railem, nie drugim zestawem głównych kart.** Jeśli `Accumulated / Distributed / Net tilt / Leader pair` dostają ciężkie boxy i dużo pionowego miejsca, zaczynają konkurować z tabelami. Lepszy wzorzec to niski, wspólny rail z krótkim meta copy i równą wysokością wszystkich insightów.
- **`Whale Transfers` korzysta z tej samej zasady co `Flows`: summary ma wspierać feed, nie dominować sekcję.** Najlepiej działa niski rail typu `Scope / Buys / Sells / Lead signal` z krótkimi podpisami. Jeśli summary w whale zaczyna przypominać duże KPI cards, feed poniżej traci hierarchię i całość robi się cięższa wizualnie.
- **W `Whale Transfers` toolbar filtrów działa najlepiej pod summary railem, nie nad nim.** Najpierw użytkownik powinien przeczytać `Scope / Buys / Sells / Lead signal`, a dopiero potem dostać `rows + All / Buys / Sells / Moves` tuż nad tabelą. To daje naturalniejszy przepływ sekcji i nie miesza warstwy insightów z warstwą sterowania.
- **W gęstych tabelach overview data powinna dostać własną kolumnę, jeśli użytkownik ma po niej sortować.** Doklejanie daty pod `From` wygląda oszczędnie, ale utrudnia skanowanie i robi wrażenie pół-funkcji. W `Coinbase Prime Transfers` lepiej zadziałał osobny `Date` column z klikanym sortem i prostszy `From` cell z samym hashem transakcji pod adresem.
- **Nie każda tabela overview potrzebuje `% of supply` jako osobnej kolumny.** W `Coinbase Prime Investors` ten sygnał był wtórny wobec `Balance`, a tylko poszerzał tabelę. Lepiej zostawić ten kontekst w górnych statystykach i utrzymać samą listę wallets bardziej zwartą.
- **Najlepsza spójność dashboardu przychodzi wtedy, gdy wszystkie sekcje tabelowe dzielą ten sam “język rails”.** Jeśli `overview`, `Flows` i `Whale` mają różne nagłówki, inne odsadzenie summary stripów i inny rytm toolbarów, całość wygląda jak kilka osobnych modułów. Warto trzymać wspólny wzorzec: ten sam header z ikoną, ten sam inset summary rail i ten sam control rail pod nim.
- **W głównych wierszach discovery najlepiej zostawić maksymalnie dwa badge’e.** Jeden główny dla encji lub fresh statusu i ewentualnie drugi pomocniczy dla naprawdę ważnego kontekstu (`Coinbase`, `Strategic`) daje dużo lepszą czytelność niż dokładanie `type + cohort + source + profile` naraz. Reszta powinna żyć w drawerze szczegółów.
- **Wallet, który 1:1 forwarduje środki do oznaczonego `Coinbase Prime`, warto oznaczać ręcznie jako `Likely Coinbase Routing Wallet`, ale nie jako twardy `CEX`.** To lepiej oddaje heurystykę: routing/custody proxy dla Coinbase Prime bez nadmiernego udawania pewności, że to oficjalny hot wallet giełdy.
- **Fallback CSV w pipeline musi być odporny na ucięte lub niepełne wiersze.** Explorerowe eksporty potrafią zawierać rekord z samym adresem bez `Balance`, więc parser nie może zakładać, że `row.get("Balance")` zawsze zwróci string. Lepiej normalizować pola defensywnie i raportować liczbę pominiętych uszkodzonych wierszy niż wywalać cały GitHub Actions run.
- **Snapshot musi być domykany przez jedną kanoniczną normalizację po wszystkich mutacjach danych.** Jeśli workflow wykona `generate_flows`, `monitor_whale_transfers` albo `detect_fresh`, a potem nie odpali `sanitize_zro_data.py`, bardzo łatwo o cichy dryf: stare `meta.integrity`, niezsynchronizowane `chains.supply`, rozjazd między `whale_transfer_diagnostics` a realną tabelą i inne stany, które przejdą proste smoke testy.
- **`zro_data.json` i `holders_multichain.json` nie mogą być traktowane jako dwa luźne snapshoty.** Jeśli jeden plik staje się kanonicznym źródłem labeli, typów i fresh metadata, drugi musi być po finalnym pipeline zsynchronizowany do tego samego universe holderów. Inaczej kolejne kroki incremental refreshu i ręczne audyty pracują na dwóch różnych wersjach prawdy.
- **Testy snapshotu powinny sprawdzać zgodność diagnostyki z realnymi rekordami, nie tylko obecność pól.** Samo istnienie `flow_diagnostics` lub `whale_transfer_diagnostics` nie wystarcza; trzeba porównywać liczniki z faktyczną zawartością `flows` i `whale_transfers`, bo dopiero to łapie cichy dryf pipeline’u.
- **Workflow po `sanitize_zro_data.py` powinien mieć jeszcze twardą walidację i stage wszystkich zsynchronizowanych plików.** Jeśli normalize zapisuje też `holders_multichain.json`, a hourly workflow patrzy tylko na `zro_data.json`, drift wróci mimo poprawnego kodu. Bezpieczny wzorzec to: `sanitize_zro_data.py --fail-on-anomaly` → `verify_palace_rules.py` → `npm test` → dopiero commit.
- **Gate po normalizacji powinien failować tylko na problemach, które zostały po naprawie, nie na tych, które sanitizer już skutecznie usunął.** Jeśli `duplicate_records_removed > 0`, ale końcowy snapshot jest już spójny, workflow nie powinien blokować publikacji świeżych danych tylko dlatego, że wejście było brudne. Lepiej failować na pozostałych anomaliach (`chain_balance_anomalies`, brakujących fresh metadata itp.). 
- **`fresh_wallets_missing_last_flow` to dobry wskaźnik jakości, ale niekoniecznie dobry hard blocker dla CI.** Przy ograniczeniach coverage/API można mieć poprawny i użyteczny snapshot z pojedynczym `Fresh Wallet`, dla którego nie udało się odzyskać ostatniego transferu ZRO. To warto raportować i monitorować, ale nie zawsze warto na tym zatrzymywać cały publish pipeline.
- **Log `❌` w CI nie powinien oznaczać “soft skip”.** Jeśli opcjonalny lokalny plik, taki jak `.mempalace-brain.yaml`, nie istnieje na GitHub runnerze, skrypt powinien wypisać jasny `warning/skip`, a nie czerwony pseudo-błąd. Inaczej bardzo łatwo pomylić hałas w logach z prawdziwą przyczyną nieudanego workflow.
