...
const CBT_COLUMNS_DEF = [
    { id:'date_tx', header:'Date / TX', width:100, minW:70, align:'left', render: (t) => { const d=new Date(t.timestamp*1000); const ds=d.toLocaleDateString('en-US',{month:'short',day:'numeric'}); const ts=d.toLocaleTimeString('en-US',{hour:'2-digit',minute:'2-digit',hour12:false}); return `<div style="font-size:12px;line-height:1.4"><span class="val-muted">${ds} ${ts}</span><br><a href="https://etherscan.io/tx/${t.hash}" target="_blank" rel="noopener" style="color:var(--accent-cyan);font-size:10px">${t.hash.slice(0,10)}…</a></div>`; }},
